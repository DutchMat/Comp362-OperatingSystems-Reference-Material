...
SIMFS_ERROR simfsMountFileSystem(char *simfsFileName)
{
    ...
    fclose(file);
    return traverse(simfsVolume->superblock.attr.rootNodeIndex);
...
SIMFS_ERROR simfsCreateFile(SIMFS_NAME_TYPE fileName, SIMFS_CONTENT_TYPE type)
{
	...
	if(simfsGetFileInfo(fileName, NULL, NULL, &block) == SIMFS_NO_ERROR){
	printf("I'm already here!");
	...
	printf("%s",simfsVolume->block[block->nodeReference].content.fileDescriptor.name);
	simfsInitializeBlock(&(block->nodeReference), &type);
    // TODO done: implement
	printf("I'm here papa, now! doing the best I can");
    return SIMFS_NO_ERROR;
}
...
SIMFS_ERROR simfsDeleteFile(SIMFS_NAME_TYPE fileName)
{
	...
	if(simfsGetFileInfo(fileName, NULL, &block, &prev) == SIMFS_NOT_FOUND_ERROR){
	printf("I removed not goodly");
	return SIMFS_NOT_FOUND_ERROR;
	}
	simfsClearBit(simfsVolume->bitvector, block->nodeReference);
	simfsClearBit(simfsContext->bitvector, block->nodeReference);
	if(prev != NULL){
	prev->next = block->next;
	}
	else{
	simfsContext->directory[hash(fileName)] = block->next;
	}
	free(block);
	printf("I removed goodly");
    return SIMFS_NO_ERROR;
}
